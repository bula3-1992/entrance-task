...
var ev, current_time, monthNames = ["января", "февраля", "марта", "апреля", "мая", "июня", "июля",
...
function renderNewInput(data, elementToRender){	
	...
	$('#form__contacts').slimScroll({height: '102px',width: '420px'});
	httpPostAsync("{users{ id, login, homeFloor, avatarUrl}}", renderContacts, element_users);
	changeRecomendations();
	footer = document.getElementById('footer');
	footer.innerHTML+='<a class="footer__link" href="#" onclick="submit_create()"><span class="font__newmeeting">Создать встречу</span></a>';
...
function renderInput(data, elementToRender){	
	...
	if(ev){
		...
		footer = document.getElementById('footer');
		footer.innerHTML+='<a class="footer__link grey" href="#" onclick="showPopup2()"><span class="font__newmeeting black">Удалить встречу</span></a>';
		footer.innerHTML+='<a class="footer__link grey" href="#" onclick="submit_edit()"><span class="font__newmeeting black">Сохранить</span></a>';
		$("#element_timeStart").mask("99:99");
		$("#element_timeEnd").mask("99:99");
		checkClear(document.getElementById('element_title'));
	}
...
function submit_create(){	
	...
	xhr.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if(createEvent){
				location.replace('/success?c_' + createEvent.id);
			}else{
				alert("Ошибка");
			}
		}
	...
	xhr.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			...
			if(updateEvent && changeEventRoom){
				location.replace('/success?e_' + ev.id);
			}else{
				alert("Ошибка");
			}
		}
